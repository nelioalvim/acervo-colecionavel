export default async function handler(req, res) {
  ...
  const prompt = `Você é um especialista em games e eletrônicos retrô colecionáveis. Pesquise o preço atual de mercado no Brasil para o item: "${itemName}". Foque exclusivamente no Mercado Livre Brasil (BRL). Use até 4 buscas, variando os termos se necessário (remova "(CIB)"/"(Loose)", tente só o nome principal). Pode estimar com base em itens similares se não achar preço exato. Não é erro não encontrar — use null só após tentar variações.
